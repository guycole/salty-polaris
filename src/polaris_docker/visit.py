...
class VisitDriver:
    ...
    def visit_insert(self, vessel_dict: dict[str, any]) -> None:
        ...
        args = {
            ...
            "locode_destination": detail_dict["observation"].get(
                "destinationLoCode", "XXXXX"
            ),
    ...
    def visit_update(self, vessel_dict: dict[str, any]) -> None:
        ...
        args = {
            ...
            "date_departure": vessel_dict["departure_date"],
            "duration_days": (
                vessel_dict["departure_date"] - vessel_dict["arrival_date"]
            ).days,
            "imo_code": vessel_dict["imo_code"],
            "locode_destination": detail_dict["observation"].get(
                "destinationLoCode", "XXXXX"
    ...
    def observation_insert(
        self, lo_code: str, obs_date: datetime.datetime, vessel: dict[str, any]
    ) -> None:
    ...
    def visit_v1(self, json_dict: dict[str, any]) -> None:
        ...
        for key in vessels:
            vessel = vessels[key]
            print(
                f"{vessel['name']} ({vessel['imo_code']}) {vessel['gross_ton']} {vessel['in_port']} {vessel['arrival_date']} {vessel['departure_date']}"
            )
            ...
            selected = self.postgres.visit_select_for_duplicate(
                vessel["imo_code"], vessel["arrival_date"], vessel["departure_date"]
            )
            if len(selected) < 1:
...
if __name__ == "__main__":
    ...
    with open(file_name, "r") as in_file:
        try:
            ...
            driver.execute(
                "test", "/var/polaris/fresh/fa94efb1-8d06-4aed-b5a8-f1e6ea635f49.html"
            )
